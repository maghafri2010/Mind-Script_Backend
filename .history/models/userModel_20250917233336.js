...
export const profile_Edit = async(username, firstname, lastname, email, phone, created_at, id) => {
    const query = "UPDATE users SET username = ?, firstname = ?, lastname = ?, email = ?, phone = ?, created_at = ? WHERE user_id = ? ";
    const [result] = await db.execute(query, [username, firstname, lastname, email, phone, created_at, id]);
    return result;
};
